...
def generate_cpu(out_file="generated_distilgpt.txt"):
    ...
    with open(out_file, "w", encoding="utf-8") as f:
        for i, prompt in enumerate(PROMPTS):
            for j in range(params.get("num_return_sequences", 3)):
                results = gen(prompt, **params)
                # Each call returns a list of dicts [{"generated_text": "..."}]
    ...
    print(f"[INFO] Wrote generated samples to: {out_file}")
